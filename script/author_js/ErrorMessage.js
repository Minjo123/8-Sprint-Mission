...
function applyNoValueErrorMessage(target, errorMessage) {
  switch (target.name) {
    case "email":
...
function applyNotValidErrorMessage(target, errorMessage) {
  switch (target.name) {
    case "email":
...
export function createErrorMessage(target, valid) {
  ...
  if (!target.value) {
    applyNoValueErrorMessage(target, errorMessage);
    return;
  }
  if (!valid) {
    applyNotValidErrorMessage(target, errorMessage);
    return;
  }
}
